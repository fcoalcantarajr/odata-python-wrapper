...
if not pat or not org or not project:
    ...
    if not pat or not org or not project:
        print("FATAL: missing ADO env vars", file=sys.stderr)
        sys.exit(1)
...
async def main() -> None:
    ...
    from ado_odata_async import __version__ as lib_version
    ...
    async with AdoODataClient(org=org, project=project, pat=pat) as client:
        ...
        async for page in client.paginate(
            ...
            query={
                ...
                "$select": (
                    "WorkItemId,Title,State,WorkItemType,"
                    "CreatedDate,ActivatedDate,ClosedDate,StateChangeDate"
                ),
        ...
        if len(items) < 10:
            print(f"FATAL: only {len(items)} items", file=sys.stderr)
            sys.exit(1)

    os.makedirs("out", exist_ok=True)
    with open("out/raw_sample.json", "w") as f:  # noqa: ASYNC230
    ...
    # Cycle time: ActivatedDate→ClosedDate (first active→closed).
    # Drop zero-day items (NOT zeros per DoD).
    vals = sorted(
        [
            round((cd - ad).total_seconds() / 86400, 2)
            for i in items
            if (ad := parse_dt(i.get("ActivatedDate")))
            and (cd := parse_dt(i.get("ClosedDate")))
            and cd > ad
    ...
    def pct(p):
        return vals[min(cc - 1, int(cc * p / 100))] if cc else 0.0

    cycle_time = dict(
        count=cc, median_days=pct(50), p50=pct(50), p85=pct(85), p95=pct(95), values_days=vals
    )
    ...
    for i in range(30):
        ...
        wip[ds] = sum(
            1
            for item in items
            if (c := parse_dt(item.get("CreatedDate")))
            and c.date() <= d
    ...
    with open("out/metrics.json", "w") as f:  # noqa: ASYNC230
