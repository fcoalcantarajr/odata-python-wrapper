...
- **HR-18** **`git-keeper` é o dono preferencial do git** (single-owner no happy-path). Outros agentes imprimem `[GIT_REQUEST] <msg>` e param. **Fail-fast handoff (resiliência que trava o projeto é teimosia):** em QUALQUER falha do `git-keeper` (rate limit, auth, rede, hang, exit≠0 — de qualquer tipo), no máximo **1 retry curto** só pra falha transiente (rate limit/rede); falha não-transiente (auth, exit≠0 determinístico) faz handoff já na **1ª** ocorrência. Na **2ª falha de qualquer tipo**, o `git-keeper` para, emite `[GIT_HANDOFF: <causa>]` e **repassa a tarefa git pedida pro PRIMARY/sisyphus** (lane de modelo própria, fora de `.opencode/agents/`), que conclui o passo mecânico pendente com os 4 gates GREEN obrigatórios. Sem backoff longo, sem circuit breaker que estaciona o loop, sem retry infinito. **Nunca** delega git pra outro *subagent* (HR-17 + audit grep impediriam) — o handoff é sempre pro orchestrator. Audit: `grep -rnE '\\bgit (commit|push|pull|merge|rebase|add|reset|checkout|tag)\\b' .opencode/agents/ | grep -v git-keeper.md` deve ser vazio.
